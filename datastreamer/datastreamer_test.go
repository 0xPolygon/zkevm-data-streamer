...
func TestServer(t *testing.T) {
	...
	streamServer, err = datastreamer.NewServer(config.Port, 1, 137, streamType, config.Filename, config.WriteTimeout, config.InactivityTimeout, 5*time.Second, &config.Log) // nolint:gomnd
	if err != nil {
		panic(err)
	}
