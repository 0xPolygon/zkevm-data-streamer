...
func NewServer(port uint16, version uint8, chainID uint64, streamType StreamType, fileName string, cfg *log.Config) (*StreamServer, error) {
	...
	var err error
	s.streamFile, err = NewStreamFile(s.fileName, version, chainID, s.streamType)
	if err != nil {
		return nil, err
	}
