...
func TestEntityToString(t *testing.T) {
	...
	expected := fmt.Sprintf("EffectiveGasPricePercentage: %d, IsValid: %d, EncodedLength: %d, Encoded: %s", l2Transaction.EffectiveGasPricePercentage, l2Transaction.IsValid, l2Transaction.EncodedLength, common.Bytes2Hex(l2Transaction.Encoded))
	assert.Equal(t, expected, stringValue)

	stringValue = entityDef.ToString(l2Transaction2.Encode())
	expected = fmt.Sprintf("EffectiveGasPricePercentage: %d, IsValid: %d, EncodedLength: %d, Encoded: %s", l2Transaction2.EffectiveGasPricePercentage, l2Transaction2.IsValid, l2Transaction2.EncodedLength, common.Bytes2Hex(l2Transaction2.Encoded))
	assert.Equal(t, expected, stringValue)
}
